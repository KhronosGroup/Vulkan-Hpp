...
std::string readTypePrefix(tinyxml2::XMLNode const* node)
{
  ...
  if (node && node->ToText())
  {
    ...
    check((prefix == "const") || (prefix == "const struct") || (prefix == "struct") || (prefix == "typedef"), node->GetLineNum(), "unexpected prefix <" + prefix + ">; expected <const>, <const struct>, <struct>, or <typedef>");
  }
  return prefix;
}
...
void VulkanHppGenerator::readCommand(tinyxml2::XMLElement const* element, std::map<std::string, std::string> const& attributes)
{
  ...
  for (auto child : children)
  {
    ...
    if (value == "param")
    {
      readCommandParam(child, commandData.params);
    }
    else if (value == "proto")
    {
...
void VulkanHppGenerator::readCommandParam(tinyxml2::XMLElement const* element, std::vector<ParamData> & params)
{
  int line = element->GetLineNum();
  std::map<std::string, std::string> attributes = getAttributes(element);
...
void VulkanHppGenerator::readExtension(tinyxml2::XMLElement const* element)
{
  ...
  for (auto const& attribute : attributes)
  {
    ...
    {
      check(std::find_if(m_features.begin(), m_features.end(), [&requires = attribute.second](std::pair<std::string, std::string> const& nameNumber){ return nameNumber.second == requires; }) != m_features.end(),
...
void VulkanHppGenerator::readTypeEnum(tinyxml2::XMLElement const* element, std::map<std::string, std::string> const& attributes)
{
  ...
  if (alias.empty())
  {
    m_enums.insert(std::make_pair(name, EnumData()));
