...
struct AliasData
{
  AliasData(std::string const& v, std::string const& p)
    : value(v)
    , protect(p)
  {}
...
struct VkData
{
  ...
  std::set<std::string>                         aliasedStructs;  // structs which are defined through an alias
...
void aliasEnum(VkData & vkData, std::map<std::string, EnumData>::iterator enumsIt, std::string const& newName, std::string const& protect)
{
  ...
  {
    vkData.aliases.insert(std::make_pair(newName, AliasData(enumsIt->first, protect)));
    ...
    vkData.dependencies.back().dependencies.insert(enumsIt->first);
  }
  else
  {
    ...
    assert((dependencyIt != vkData.dependencies.end()) && (dependencyIt->dependencies.size() == 1) && (dependencyIt->dependencies.find(enumsIt->first) != dependencyIt->dependencies.end()));
  }
}
        ...
        else
        {
          ...
          else
          {
            ...
            if (structsIt != vkData.structs.end())
            {
              ...
              vkData.aliasedStructs.insert(strippedName);
              vkData.vkTypes.insert(strippedName);
            }
            else
            {
...
void readType(tinyxml2::XMLElement * element, VkData & vkData)
{
  ...
  if (categoryIt != attributes.end())
  {
    ...
    {
      assert(categoryIt->second == "union");
...
void writeTypeAlias(std::ostream & os, VkData const& vkData, DependencyData const& dependencyData)
{
  ...
  assert(vkData.enums.find(aliasIt->second.value) != vkData.enums.end());
...
void writeStructureChainValidation(std::ostream & os, VkData const& vkData, DependencyData const& dependencyData)
{
  ...
  if (!it->second.structExtends.empty()) {
    ...
    for (auto extendName : it->second.structExtends)
    {
      ...
      //if (itExtend == vkData.structs.end() && vkData.aliases.find(extendName) != vkData.aliases.end())
      if (itExtend == vkData.structs.end() && vkData.aliasedStructs.find(extendName) != vkData.aliasedStructs.end())
      {
        continue;
      }
...
{
  bool found = false;
  for (auto d : vkData.dependencies) {
    if (d.name == "getBufferMemoryRequirements2KHR") {
      found = true;
    }
  }
  assert(found);
