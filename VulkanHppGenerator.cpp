                                             ...
                                             MemberData const &  memberData ) const
{
  if ( memberData.type.type != "VkStructureType" )  // filter out StructureType, which is supposed to be immutable !
  {
    ...
    str +=
      "\n"
      "    " +
      structureName + " & set" + startUpperCase( memberData.name ) + "( " + memberType + " " + memberData.name +
      "_ ) VULKAN_HPP_NOEXCEPT\n"
      "    {\n"
      "      ";
...
void VulkanHppGenerator::appendUnion( std::string & str, std::pair<std::string, StructureData> const & structure ) const
{
  ...
  for ( auto const & member : structure.second.members )
  {
    ...
    memberType       = ( member.arraySizes.empty() )
                   ? member.type.compose()
...
namespace std
{
  ...
  try
  {
    ...
    {
      std::cout << "VkGenerate: failed to load file " << filename << " with error <" << to_string( error ) << ">"
                << std::endl;
      return -1;
    }
    ...
    if ( ret != 0 )
    {
      std::cout << "VkGenerate: failed to format file " << filename << " with error <" << ret << ">\n";
      return -1;
    }
#endif
  }
