...
namespace
{
  ...
  template <typename T>
  typename std::vector<T>::const_iterator find( std::vector<T> const & values, std::string const & name );
  template <typename T>
  typename std::vector<T>::iterator find( std::vector<T> & values, std::string const & name );
...
{
  return std::any_of( enumValues.begin(), enumValues.end(), [&name]( EnumValueData const & ev ) { return ev.name == name; } );
}
                                         ...
                                         std::map<std::string, EnumData>::const_iterator stageFlagBits2It )
{
  ...
  for ( auto const & attribute : attributes )
  {
    if ( attribute.first == "alias" )
    {
      ...
      aliasIt   = find( accessFlagBitsIt->second.values, attribute.second );
      checkForError(
    ...
    else
    {
      ...
      auto nameIt = find( accessFlagBits2It->second.values, attribute.second );
      checkForError(
                                        ...
                                        std::map<std::string, EnumData>::const_iterator stageFlagBits2It )
{
  ...
  for ( auto const & attribute : attributes )
  {
    if ( attribute.first == "alias" )
    {
      ...
      aliasName = attribute.second;
      aliasIt   = find( stageFlagBitsIt->second.values, attribute.second );
      checkForError(
    ...
    {
      assert( attribute.first == "name" );
      auto nameIt = find( stageFlagBits2It->second.values, attribute.second );
      checkForError(
...
{
  auto aliasIt = find( valueAliases, name );
